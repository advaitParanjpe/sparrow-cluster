module l1_data_cache #(
  ...
  state_t state;
  logic saved_write, pending_store_after_refill, snoop_seen;
  logic [ADDR_WIDTH-1:0] saved_addr, victim_base;
  ...
  always_ff @(posedge clk) begin
    if(!rst_n) begin
      state<=IDLE; saved_write<=0; pending_store_after_refill<=0; saved_addr<=0; victim_base<=0; saved_wdata<=0; saved_wstrb<=0; saved_set<=0; saved_tag<=0; saved_word<=0; saved_way<=0; victim_way<=0; pending_cmd<=BUS_RD; response_data<=0; snoop_seen<=0;
      ...
      if(state==IDLE&&cpu_req_valid&&cpu_req_ready) begin
        ...
        if(is_uncached(cpu_req_addr)) begin uncached_access_count<=uncached_access_count+1'b1; state<=UNC_REQ; end
          ...
          if(!cpu_req_write) begin
            ...
            response_data<=data_array[hit_way][cpu_req_addr[OFFSET_BITS+SET_BITS-1:OFFSET_BITS]][cpu_req_addr[OFFSET_BITS-1:2]]; state<=RESP;
  ...
  always_ff @(posedge clk) if(rst_n) begin
    ...
    assert(!(state==RESP && saved_write && !is_uncached(saved_addr) && pending_cmd!=BUS_UPGR && pending_cmd!=BUS_RDX && msi_array[saved_way][saved_set]!=MSI_M)) else $error("L1D store completed outside M");

...
`make sim-snoop-transport` covers no-peer/shared/modified-owner reads, `BUS_RDX`, `BUS_UPGR`, writeback, four-word memory transfers, owner-to-SRAM data identity, and missing-ack timeout/error accounting. Test snoopers use the production-shaped snoop interface.

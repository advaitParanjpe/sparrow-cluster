...
.PHONY: check docs-check tree milestone-check milestone-run milestone-status sim-unit sim-l1i sim-l1d sim-cluster sim-multicore regress
IVERILOG ?= iverilog
VVP ?= vvp
...
regress: check docs-check sim-unit sim-l1i sim-l1d sim-cluster sim-multicore

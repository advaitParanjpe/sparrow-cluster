...
| `0x00003000..0x000035ff` | cacheable SparrowML package-reference data, outputs, partials, and read-only summaries |
...
The SparrowML package files occupy 53,296 committed text bytes, while the generated runtime data footprint for simulation is a compact cacheable reference region below 64 KiB SRAM. The original SparrowML package memory map reports 528 bytes of scratchpad/model runtime data; binary mirrors are intentionally not imported because the generated fallback consumes the JSON references directly.

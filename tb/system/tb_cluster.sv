module tb_cluster;
  ...
  initial begin
    ...
    // Read mhartid, store hart*4, then cycle through three code blocks mapping to set 0.
    ...
    {dut.u_mem.mem[27],dut.u_mem.mem[26],dut.u_mem.mem[25],dut.u_mem.mem[24]}=32'h3e80006f;
    {dut.u_mem.mem[1027],dut.u_mem.mem[1026],dut.u_mem.mem[1025],dut.u_mem.mem[1024]}=32'h4000006f;
    {dut.u_mem.mem[2051],dut.u_mem.mem[2050],dut.u_mem.mem[2049],dut.u_mem.mem[2048]}=32'h819ff06f;
    ...
    for (i=0; i<4; i=i+1) begin
      if (dut.l1_accesses[i*64 +: 64] !== dut.l1_hits[i*64 +: 64] + dut.l1_misses[i*64 +: 64]) $fatal(1,"L1I counter consistency hart %0d",i);
      if (dut.l1_hits[i*64 +: 64] == 0 || dut.l1_misses[i*64 +: 64] < 4 || dut.l1_refill_words[i*64 +: 64] != 4*dut.l1_misses[i*64 +: 64]) $fatal(1,"L1I refill accounting hart %0d",i);
      $display("L1I hart %0d accesses=%0d hits=%0d misses=%0d refill_words=%0d", i, dut.l1_accesses[i*64 +: 64], dut.l1_hits[i*64 +: 64], dut.l1_misses[i*64 +: 64], dut.l1_refill_words[i*64 +: 64]);
    end

...
`l1_instruction_cache` preserves the IMEM contract. `l1_data_cache` preserves the DMEM contract on its CPU side: it accepts one request, returns exactly one response, and leaves load extension to Sparrow-V. Uncached DMEM requests keep the prior word-oriented lower interface. Cacheable DMEM requests use the internal block coherence requester/snooper interface and do not alter the audited core contract.

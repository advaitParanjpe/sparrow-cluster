...
Milestone 8 SparrowML workloads also add no coherence command. The shared read-only package-reference region is read by all active cores, output regions are cacheable writes, and shared-work reduction uses per-core cacheable partial/completion slots followed by a core-0 reduction. The poor-layout experiment changes only output placement; the logical prediction/reference work remains the same as the safe-layout run.
